...
use crate::query::executor::btree_maintenance::rebuild_btree_index_with_tx;
use crate::query::executor::{Executor, TxExecutionContext};
...
impl Executor for CreateIndexExecutor {
    ...
    fn next(&mut self) -> Result<Option<Vec<Value>>> {
        ...
        if let Some(table) = self.catalog.get_table(&self.table_name) {
            if matches!(created.index_type, IndexType::BTree) {
                rebuild_btree_index_with_tx(&self.catalog, &table, &created, self.tx.as_ref())?;
            }
